...
const GROUP_BG = { "Growth": "#FAFAFA", "E&D": "#F7F7F2", "Corporate": "#F5F5FA", "Strategy": "#F9F9F9" };

const CSS = `
@keyframes burstP { 0% { transform:scale(0); opacity:1 } 50% { opacity:1 } 100% { transform:scale(1); opacity:0 } }
...
const CelebBurst = ({ show }) => {
  ...
  const particles = Array.from({ length: 12 }, (_, i) => {
    const angle = (i / 12) * 360, dist = 30 + Math.random() * 30;
    const x = Math.cos(angle * Math.PI / 180) * dist, y = Math.sin(angle * Math.PI / 180) * dist;
    return (<div key={i} style={{ position: "absolute", left: "50%", top: "50%", width: 3 + Math.random() * 4, height: 3 + Math.random() * 4, borderRadius: "50%", background: i % 3 === 0 ? "#000" : i % 3 === 1 ? "#666" : "#CCC", transform: `translate(${x}px, ${y}px) scale(0)`, animation: `burstP 0.6s cubic-bezier(0.16,1,0.3,1) ${i * 25}ms forwards` }} />);
  });
  return (<div style={{ position: "fixed", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", pointerEvents: "none", zIndex: 200 }}><div style={{ position: "relative" }}>{particles}
    <svg width="48" height="48" viewBox="0 0 48 48" style={{ animation: "fadeScale 0.4s cubic-bezier(0.16,1,0.3,1) forwards" }}><circle cx="24" cy="24" r="22" fill="#000" /><path d="M14 24L21 31L34 17" fill="none" stroke="#fff" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" style={{ strokeDasharray: 40, strokeDashoffset: 40, animation: "checkDraw 0.4s 0.2s forwards" }} /></svg>
  </div></div>);
};

const ProgressRing = ({ current, total }) => {
  const pct = current / total, r = 14, circ = 2 * Math.PI * r;
  return (
...
const Slide = ({ children, dir }) => {
  ...
  return (<div style={{ opacity: v ? 1 : 0, transform: v ? "translateY(0) scale(1)" : `translateY(${dir === "back" ? "-16px" : "16px"}) scale(0.99)`, transition: "all 0.45s cubic-bezier(0.16,1,0.3,1)", width: "100%", maxWidth: 600, margin: "0 auto" }}>{children}</div>);
};
...
const Btn = ({ children, onClick, variant = "primary", disabled, style: s }) => {
  ...
  return <button style={{ ...(variant === "primary" ? primary : ghost), ...s }} onClick={disabled ? undefined : onClick} onMouseEnter={() => setHover(true)} onMouseLeave={() => setHover(false)}>{children}</button>;
};

const Key = ({ label, dark }) => (
  <span style={{ fontSize: 10, color: dark ? "rgba(0,0,0,0.3)" : "rgba(255,255,255,0.45)", marginLeft: 4 }}>
    <kbd style={{ background: dark ? "rgba(0,0,0,0.06)" : "rgba(255,255,255,0.15)", borderRadius: 3, padding: "1px 5px", fontSize: 10, fontFamily: "monospace" }}>{label}</kbd>
  </span>
);
        ...
        {[...Array(11)].map((_, i) => {
          ...
          return (
            <button key={i} onClick={() => onSelect(i)} onMouseEnter={() => setHovered(i)} onMouseLeave={() => setHovered(null)}
              style={{ flex: "1 1 0", minWidth: 0, height: 44, borderRadius: 10, border: `${active ? 2 : 1}px solid ${border}`, background: bg, color: active ? "#fff" : "#000", fontSize: 14, fontWeight: 700, cursor: "pointer", transition: "all 0.2s cubic-bezier(0.16,1,0.3,1)", transform: active ? "scale(1.1)" : isHov ? "scale(1.05) translateY(-2px)" : "scale(1)", boxShadow: active ? "0 4px 16px rgba(0,0,0,0.2)" : isHov ? "0 2px 8px rgba(0,0,0,0.08)" : "none", fontFamily: "inherit", position: "relative", padding: 0 }}>
              {i}
              {active && <div style={{ position: "absolute", inset: -5, borderRadius: 14, border: "2px solid rgba(0,0,0,0.1)", animation: "pulseRing 1s ease-out" }} />}
            </button>
          );
        })}
      </div>
    ...
    <div style={{ background: "#FAFAFA", borderRadius: 14, padding: "18px 22px", marginBottom: 24, border: "1px solid #F0F0F0", animation: "fadeIn 0.4s ease" }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "6px 0", borderBottom: "1px solid #F0F0F0" }}>
          <span style={{ fontSize: 13, color: "#666" }}>Strategy alignment</span>
          <span style={{ fontSize: 14, fontWeight: 700, color: getNPSType(stratRating) === "promoter" ? "#2E7D32" : getNPSType(stratRating) === "passive" ? "#F57F17" : "#C62828" }}>{stratRating}/10</span>
        </div>
        {rated.map(t => (
          <div key={t.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "6px 0", borderBottom: "1px solid #F0F0F0" }}>
            ...
            <span style={{ fontSize: 14, fontWeight: 700, color: getNPSType(answers[t.id].rating) === "promoter" ? "#2E7D32" : getNPSType(answers[t.id].rating) === "passive" ? "#F57F17" : "#C62828" }}>{answers[t.id].rating}/10</span>
          </div>
        ))}
...
export default function App() {
  ...
  const currentGroup = phase === "strategy" ? "Strategy" : team?.group || "Corporate";
  ...
  const celebrate = () => { setShowCeleb(true); setTimeout(() => setShowCeleb(false), 800); };
  ...
  const handleWorked = (worked) => {
    ...
    } else { celebrate(); setTimeout(() => goNext(), 500); }
  };
        ...
        <Slide dir="forward" key="welcome">
          <div style={{ textAlign: "center", maxWidth: 480 }}>
            ...
            <div style={{ animation: "floatIn 0.6s 0.8s both" }}>
              ...
              <p style={{ fontSize: 14, color: "#999", margin: "0 0 4px", fontWeight: 500 }}>{quarter} · Quarterly NPS Survey</p>
      ...
      <div style={{ minHeight: "100vh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 24, background: "#fff" }}>
        ...
        <Slide dir="forward" key="thanks">
          <div style={{ textAlign: "center", maxWidth: 440 }}>
            ...
            <h1 style={{ fontSize: 32, fontWeight: 800, color: "#000", margin: "0 0 8px", letterSpacing: -0.5 }}>Thank you{userName !== "Team Member" ? `, ${userName.split(" ")[0]}` : ""}!</h1>
            ...
            <div style={{ background: "#FAFAFA", borderRadius: 14, padding: "20px 24px", border: "1px solid #F0F0F0", textAlign: "center" }}>
              <p style={{ fontSize: 14, fontWeight: 700, color: "#000", margin: "0 0 6px" }}>What happens next?</p>
              <p style={{ fontSize: 13, color: "#666", lineHeight: 1.6, margin: 0 }}>Results will be compiled and shared with team leads in the coming weeks.</p>
            </div>
          </div>
        </Slide>
      </div>
      ...
      <div style={{ minHeight: "100vh", display: "flex", flexDirection: "column", padding: "56px 24px 24px", background: bgColor, transition: "background 0.5s ease" }}>
        ...
        <Slide dir={slideDir} key="strategy">
          <div style={{ maxWidth: 560, margin: "0 auto" }}>
            ...
            <p style={{ fontSize: 13, color: "#CCC", margin: "0 0 28px" }}>Let's start with the big picture.</p>
          ...
          {subStep === "worked" && (
            <>
              ...
              <div style={{ display: "flex", gap: 10 }}>
                ...
                <Btn onClick={() => handleWorked(false)} variant="ghost" style={{ flex: 1, padding: "16px 32px", borderRadius: 14 }}>No <Key label="N" dark /></Btn>
              </div>
            </>
          )}
